...
source = {"type":"archive", "url":
          "https://github.com/tpaviot/oce/archive/OCE-{0}.zip".format(version)}
...
def build(libpack):
    ...
    if libpack.toolchain.startswith("vc"):
        ...
        utils.run_cmd("cmake", ["-D", "OCE_INSTALL_PREFIX=" + libpack.tmp_install,
...
def install(libpack):
    ...
    files.extend(utils.move(os.path.join(libpack.tmp_install, "lib"), libpack.path,
    ...
    files.extend(utils.move(os.path.join(libpack.tmp_install, "lib", "cmake"),
