  ...
  : QMainWindow( parent, f/*WDestructiveClose*/ )
{
    ...
    pcTaskPanelView->setObjectName
        (QString::fromLatin1(QT_TRANSLATE_NOOP("QDockWidget","Task View")));
...
void MainWindow::tabChanged(MDIView* view)
{
    ...
    for (int i = 0; i < d->tabs->count(); i++) {
        if (d->tabs->tabData(i).value<QWidget*>() == view) {
            ...
            int lastIndex = cap.lastIndexOf(QString::fromLatin1("[*]"));
            if (lastIndex > 0) {
...
//#ifndef QT_NO_OPENSSL
			...
			if (urlQuery.hasQueryItem(QString::fromLatin1("sid"))) {
                urlQuery.removeQueryItem(QString::fromLatin1("sid"));
